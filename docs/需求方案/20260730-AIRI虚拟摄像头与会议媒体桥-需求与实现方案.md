...
正式原生设备后端中，Eventa 只承载启动、停止、配置、状态、错误和低频统计，不承载逐帧视频或高频 PCM；实时媒体通过原生共享缓冲和平台音频回调传递。macOS 兼容桥的独立 Swift 子进程使用带协议版本、`sessionId`、序号、时间戳和格式头的有界 PCM 块通过 Eventa 送入 Renderer；该路径不承担视频帧传输。Renderer 校验 PCM 连续性并生成标准 512 样本帧后，直接串行送入 VAD 和当前 ASR 分段，不再用独立 AudioContext 的播放时钟桥接采集时钟。
...
| `MeetingPcmPipeline` | Renderer | 下混、重采样、帧化、时间戳、连续性校验和标准帧分发 | 不决定对话提交 |
...
5. 启动仍由同一会议媒体会话原子管理。停止时先关闭 Swift 采集输入并等待子进程退出，再停止 Renderer 的 ASR/VAD、待处理标准帧和其他媒体路由；子进程不能跨会话存活。
...
1. 会议接收链路不创建播放用途的 `AudioContext`/AudioWorklet。归一化后的标准 PCM 直接进入 VAD，并在 VAD 分段期间写入同一个 Provider PCM 流；普通麦克风的 MediaStream 路径保持独立。
...
5. 标准帧严格串行处理并暴露待处理帧数和 VAD 推理耗时；VAD Worker 崩溃、模型校验失败或 PCM 连续性破坏时中止当前语音段并令 `remote-audio-in` 路由失败，不用音量算法接管。
...
| `packages/stage-ui/src/services/meeting-speech-session.ts` | 直接消费标准 PCM，串行执行 VAD，把前置补偿帧写入当前流式 ASR 分段，并维持唯一 turn-end 语义 |
| `apps/stage-tamagotchi/src/renderer/services/meeting-media-host.ts` | 校验并归一化原生 PCM，直接分发标准帧，记录回调延迟、输入电平、待处理帧和 VAD 推理耗时 |
...
- `MeetingMediaRuntime`、平台设备句柄、共享帧槽、PCM 归一化状态、待处理标准帧、VAD Worker、ASR 会话和当前语音段。
...
3. 收敛到单一标准 PCM 数据面：全声道下混、16kHz 重采样、512 样本帧，并直接串行进入 VAD/ASR。
