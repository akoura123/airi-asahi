...
Eventa 只承载启动、停止、配置、状态、错误和低频统计，不承载逐帧视频或高频 PCM。实时媒体通过原生共享缓冲和平台音频回调传递。
...
3. 配置在 `start` 时冻结为快照；运行期改动进入待应用状态，用户确认重启会话后生效。
...
4. `remote-audio-in` 使用 `loopback` 处理配置，关闭 `autoGainControl`、`echoCancellation` 和 `noiseSuppression`。普通物理麦克风继续使用现有 `microphone` 配置。
...
- 流式适配器：`startSegment` 后接收该段 PCM，允许产生 partial；`finishSegment` 后必须产生唯一 final 或明确失败。
- 批量适配器：VAD 结束后接收完整语音段，调用现有录音转录能力，返回唯一 final 或明确失败。
...
3. partial 只更新实时字幕；final 进入 `MeetingTurnAssembler`。Assembler 校验 `sessionId`、`segmentId`、`sequence` 和完成状态后生成一次 `turn-end`。
...
2. 将音频输入用途拆为 `microphone` 与 `loopback`，会议总线关闭 AGC/AEC/降噪。
